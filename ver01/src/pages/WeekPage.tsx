...
function PlanTaskRow({
  ...
  loc: PlanLocation;
  onMoveToToday: () => void;
  moveTargets?: { weeks?: string[]; months?: string[] };
  onMove?: (target: { weekStart?: string; day?: string; month?: string; year?: string }) => void;
...
function AddPlanForm({
  ...
  placeholder: string;
  onSubmit: (payload: { title: string; priority: 1 | 2 | 3; estimateMin: number | null }) => void;
}) {
    ...
    <div className="flex flex-wrap gap-2 items-center">
      ...
      <button
        onClick={() => {
          ...
          onSubmit({ title: title.trim(), priority, estimateMin: estimate ? Number(estimate) : null });
          setTitle("");
          setEstimate("");
        }}
...
function SectionTabs({ value, onChange }: { value: "year" | "month" | "week"; onChange: (v: "year" | "month" | "week") => void }) {
  const tabs: { key: "year" | "month" | "week"; label: string }[] = [
    { key: "year", label: "Year" },
      ...
      {tabs.map((t) => (
        <button
          ...
          className={
            ...
            (value === t.key ? "bg-emerald-500 text-slate-950" : "text-slate-200 hover:bg-slate-900")
          }
        >
          {t.label}
        </button>
...
export default function PlansPage() {
  ...
  const [tab, setTab] = useState<"year" | "month" | "week">("week");
